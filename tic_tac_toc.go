...
func main() {
	...
	if playWith == player {
		...
		for k = 0; k < 9; k++ {
			//If the elements are in the same row or column, the game is over. Can also be diagonal, (Abi bu parçayı döngüyle yazıcaktım ama yetişmedi.Normalde tüm durumlardan olumsuz olanları çıkaracaktım)
